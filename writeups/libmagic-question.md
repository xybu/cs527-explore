...
root@ffd44e7d4dd4:~/file# clang -v
...
For libmagic (https://github.com/file/file), if you compile with clang+ASAN
...
Compile with GCC+asan

```
CC=/usr/local/bin/afl-gcc ASM=/usr/local/bin/afl-gcc CFLAGS="-g -O0 -fsanitize=address" ./configure --disable-silent-rules --disable-shared
```

It again reports memory leak:
...
Direct leak of 30 byte(s) in 1 object(s) allocated from:
    #0 0x7f55cb72a961 in realloc (/usr/lib/x86_64-linux-gnu/libasan.so.2+0x98961)
    #1 0x40c98a in xrealloc /root/file/tests/test.c:36
    #2 0x40c98a in slurp /root/file/tests/test.c:63
    #3 0x40c98a in main /root/file/tests/test.c:101
